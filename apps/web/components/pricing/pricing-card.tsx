...
export function PricingCard({
  ...
  return (
    ...
    >
      {popular && (
        <div className="absolute top-0 left-1/2 z-70 -translate-x-1/2 -translate-y-1/2">
      ...
      <h3 className={cn("text-4xl font-bold", popular && "text-[#2EAFB4]")}>
        {title}
      </h3>
      ...
      <div className="mt-8 flex items-end gap-2">
        ...
        <span className="pb-2 text-2xl text-muted-foreground">{period}</span>
      </div>
        ...
        {features.map((feature) => (
          <li key={feature} className="flex items-center gap-3">
            ...
            {feature === "Create Invoices" && (
              <span className="text-xs text-gray-400">(5 invoices/mo.)</span>
            )}
          </li>
        ))}
      </ul>
      ...
      {buttonText === "Get Started Free" ? (
        <Button
          ...
          className={cn(
            ...
            popular ? "bg-[#2EAFB4] hover:bg-[#26989d]" : "variant-outline"
          )}
